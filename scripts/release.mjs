...
async function main() {
  if (!skipCleanGitCheck) {
    ...
    if (isDirtyGit) {
      console.log(chalk.red(`Git 仓库不干净`));
      return;
    }
  ...
  const pkgWithVersions = await pSeries(
    packagesToRelease.map(({ name, path, pkg }) => async () => {
      ...
      const { release } = await prompt({
        type: "select",
        name: "release",
        message: `为 ${chalk.bold.white(name)} 选择发布类型`,
      ...
      if (release === "custom") {
        version = (
          await prompt({
            type: "input",
            name: "version",
            message: `输入自定义版本 (${chalk.bold.white(name)})`,
            initial: version,
          })
        ).version;
      } else {
      ...
      if (!semver.valid(version)) {
        throw new Error(`无效的目标版本：${version}`);
      }
